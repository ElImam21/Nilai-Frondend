...
export default function EditPendaftarPage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 flex flex-col items-center justify-center p-4 overflow-hidden w-screen h-screen fixed inset-0">
      ...
      {isSuccess && (
        <div className="fixed inset-0 bg-black/30 backdrop-blur-lg flex items-center justify-center z-50">
          <div className="bg-white p-8 rounded-2xl shadow-2xl text-center animate-scale-in">
            <div className="w-20 h-20 bg-green-500 rounded-full flex items-center justify-center mx-auto mb-6">
              <svg xmlns="http://www.w3.org/2000/svg" className="h-12 w-12 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
              </svg>
            </div>
            <h2 className="text-2xl font-bold text-gray-800 mb-4">Data Berhasil Diperbarui!</h2>
            <p className="text-gray-600">Anda akan diarahkan kembali ke halaman daftar pendaftar.</p>
          </div>
        </div>
      )}

      {/* Form container - Lebarkan dan atur tata letak */}
      <div className={`relative z-10 w-full max-w-3xl bg-white/80 backdrop-blur-sm rounded-2xl shadow-2xl p-8 transition-all duration-1000 ease-out ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
        {/* Header dengan flex untuk penjajaran yang tepat */}
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="relative">
              ...
              <input
                ...
                className={`w-full px-4 py-3 rounded-xl border ${errors.nama ? 'border-red-500' : 'border-gray-300'} focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all duration-300`}
                placeholder="Masukkan nama lengkap"
                maxLength={15}
              />
              {errors.nama && <p className="mt-2 text-red-500 text-sm animate-pulse">{errors.nama}</p>}
            </div>

            <div className="relative">
              <label htmlFor="usia" className="block text-gray-700 font-medium mb-2">Usia</label>
              <input
                type="number"
                id="usia"
                value={usia}
                onChange={(e) => setUsia(e.target.value)}
                className={`w-full px-4 py-3 rounded-xl border ${errors.usia ? 'border-red-500' : 'border-gray-300'} focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all duration-300`}
                placeholder="Masukkan usia"
                min="17"
                max="60"
              />
              {errors.usia && <p className="mt-2 text-red-500 text-sm animate-pulse">{errors.usia}</p>}
            </div>
          </div>

          {/* Baris kedua dengan dua kolom */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="relative">
              ...
              <input
                ...
                className={`w-full px-4 py-3 rounded-xl border ${errors.email ? 'border-red-500' : 'border-gray-300'} focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all duration-300`}
                placeholder="Masukkan email"
              />
              {errors.email && <p className="mt-2 text-red-500 text-sm animate-pulse">{errors.email}</p>}
            </div>

            <div className="relative">
              <label htmlFor="nomorTelpon" className="block text-gray-700 font-medium mb-2">Nomor Telepon</label>
              <div className="flex">
                ...
                <input
                  ...
                  className={`w-full px-4 py-3 rounded-r-xl border ${errors.nomorTelpon ? 'border-red-500' : 'border-gray-300'} focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all duration-300`}
                  placeholder="081234567890"
                />
              </div>
              {errors.nomorTelpon && <p className="mt-2 text-red-500 text-sm animate-pulse">{errors.nomorTelpon}</p>}
            </div>
          </div>

          {/* Motivasi - satu kolom penuh */}
          <div className="relative">
            <label htmlFor="motivasi" className="block text-gray-700 font-medium mb-2">Motivasi</label>
            <textarea
              id="motivasi"
              value={motivasi}
              onChange={(e) => setMotivasi(e.target.value)}
              rows={4}
              className={`w-full px-4 py-3 rounded-xl border ${errors.motivasi ? 'border-red-500' : 'border-gray-300'} focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all duration-300`}
              placeholder="Tuliskan motivasi Anda"
            ></textarea>
            {errors.motivasi && <p className="mt-2 text-red-500 text-sm animate-pulse">{errors.motivasi}</p>}
          </div>
          ...
          <div className="pt-4">
            <button
              ...
              className={`w-full relative group px-6 py-4 rounded-xl shadow-xl transform hover:-translate-y-1 transition-all duration-300 cursor-pointer ${isSubmitting
                  ? 'bg-gray-400 cursor-not-allowed'
                  : 'bg-gradient-to-r from-blue-600 to-indigo-700 hover:shadow-2xl'
                }`}
            >
              <span className={`absolute inset-0 w-full h-full mt-1 ml-1 rounded-xl group-hover:mt-0 group-hover:ml-0 transition-all duration-300 ${isSubmitting ? 'bg-gray-600' : 'bg-indigo-900'
                }`}></span>
              <span className="relative flex items-center justify-center gap-2 text-white text-xl font-bold tracking-wider">
                {isSubmitting ? (
                  <>
